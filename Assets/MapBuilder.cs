    ...
    private void Awake()
    {
        ...
        for (int i=0; i< (int)(scale.x/_sizeTile); i++)
        {
            for (int j = 0; j < (int)(scale.y / _sizeTile)+1; j++)
            {
                ...
                float k = (scale.x / newTileSize.x) / (int)(scale.x / _sizeTile);

                newTile.transform.localScale = newTile.transform.localScale * k;

                newTileSize = newTileSize * k;

                Vector2 startPosition = (Vector2)gameObject.transform.position - new Vector2(-newTileSize.x / 2, newTileSize.y / 2);

                newTile.transform.position = startPosition + new Vector2(i * newTile.SpriteRenderer.bounds.size.x, -j * newTile.SpriteRenderer.bounds.size.y);
